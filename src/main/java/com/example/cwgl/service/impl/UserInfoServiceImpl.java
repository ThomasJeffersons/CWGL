...
public class UserInfoServiceImpl implements UserInfoService {
    ...
    public int add(User userInfo) {
        ...
        if (userInfo.getRoleid() == 2) {
            ...
            //添加家庭成功后，绑定到该用户
            if (r == 1) {
                userInfo.setHouseid(newHouse.getId());
    ...
    @Override
    public boolean userIsExisted(User userInfo) {
        return userInfoMapper.userIsExisted(userInfo) > 0 ? true : false;
    }

    @Override
    public Result getUsersByWhere(PageModel<User> model) {
        try {
            List<User> users = userInfoMapper.getUsersByWhere(model);
            if (users.size() >= 0) {
